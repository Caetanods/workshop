...
## Caso tenha resultados de uma análise MCMC de algum outro modelo de sua escolha, tente
##   utilizar com esse tutorial.
...
## Esse é o caminho para o diretório no meu computador:
...
## Veja a pagina de ajuda dessa função:
help( mcmc )
...
## Somente podemos combinar as cadeias de MCMC pois estamos trabalhando com o mesmo
##   modelo e com o mesmo conjunto de dados. Portanto, cada uma dessas cadeias são
##   resultados de uma mesma análise.

## Primeiramente podemos ver o 'summary' das nossas cadeias:
summary(mcmc)
## Nesse summary podemos ver uma quantidade importante. O 95% HPD (highest posterior density),
##   a região de 95% de maior densidade (i.e., probabilidade posterior) dos parâmetros.
##   Veja, por exemplo, que o valor de 'root' (valor da raíz da filogenia) varia entre
##   9.41 e 11.47 no 95% HPD. Mas ainda temos outras análises a fazer.
...
plot(x = mcmc, trace = TRUE, density = FALSE)
plot(x = mcmc, trace = FALSE, density = TRUE)
...
plot(x = post.mcmc, trace = TRUE, density = FALSE)
plot(x = post.mcmc, trace = FALSE, density = TRUE)
...
## Vamos colocar 'multivariate=FALSE' pois queremos avaliar cada um dos parâmetros
##   independentemente. Ou seja, não estamos considerando a correlação entre esses
##   parâmetros.
##   Veja outras opções no pagina de ajuda dessa função.

help(gelman.diag)

gelman.diag(x = post.mcmc, transform = FALSE, autoburnin = FALSE, multivariate = FALSE)

## Queremos que o resultado seja o mais próximo possível de '1.00'.

## As vezes esse teste vai resultar em um valor próximo de 1. Por exemplo, 1.05 ou até mesmo
##  valores maiores, próximos de 1.1. Nesse caso, sabemos que as cadeias estão quase convergindo
##  completamente. Sabemos que está tudo funcionando na nossa análise. Podemos aumentar o número
##  de gerações para conseguir uma melhor amostragem da posterior.

## OK. Retiramos o burn-in e sabemos que as cadeias convergiram. No que mais podemos
##  estar interessados?
...
## Note que usamos 'relative = FALSE', pois não aplicamos o thinning na cadeia.
...
## Esse procedimento é chamado de 'thinning' da cadeia de MCMC.
...
## Claro, diferentes casos podem necessitar diferentes numeros de ESS. No programa
##   BEAST, por exemplo, é recomendado um número mínimo de 200.
...
## Agora podemos utilizar tal distribuição para fazer conclusões sobre nosso modelo.
...
## Na prática, temos que rodar análises de MCMC com diferentes números de gerações e
##  observar o comportamento da estimativa.
